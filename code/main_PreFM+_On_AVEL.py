...
from dataloader_PreFM import PreFM_plus_UnAV_dataset
from network_PreFM import PreFM_plus_On_AVEL_Net
...
def train(args, model, train_loader, optimizer, criterion, epoch, device):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        audio, visual  = batch_data['audio'].to(device), batch_data['visual'].to(device) # B 30 1536
        curr_label = batch_data['curr_label'].float().to(device) # B 30 100
        future_label = batch_data['future_label'].float().to(device) # B 10 100
        curr_f_label, future_f_label  = batch_data['curr_f_label'].to(device), batch_data['future_f_label'].to(device) # B 30 1536   B 10 1536
        all_label = torch.cat([curr_label, future_label], dim=1) # B 40 100
        ...
        time_weights = gaussian_weights(num_steps = 30+ args.future_length, center = 29, sigma = 8).to(device)
        l_a = F.binary_cross_entropy_with_logits(all_frame_logits, all_label, reduction='none')
        loss_cls = l_a * time_weights.view(1, 30+ args.future_length, 1)
        loss_cls = loss_cls.mean()
        loss_kd_curr = kd_cos_loss(curr, curr_f, time_weights[:30])
        loss_kd_future = kd_cos_loss(future, future_f, time_weights[30:])
...
def eval(args, model, data_loader, label_dir, criterion, device, mode):
    ...
    pool = nn.AvgPool1d(kernel_size=4, stride=4, ceil_mode=True)
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            curr_label = batch_data['curr_label'].float().to(device)
            curr_f_label = batch_data['curr_f_label'].to(device)
            batch_size = visual.size(0)
            ...
            if criterion != None:
                time_weights = gaussian_weights(num_steps = 30+ args.future_length, center = 29, sigma = 8).to(device)
                loss_curr = F.binary_cross_entropy_with_logits(all_frame_logits[:,:30,:], curr_label)
                loss_kd_curr = kd_cos_loss(curr, curr_f, time_weights[:30])
            ...
            curr_prob = torch.sigmoid(all_frame_logits[:length,29,:]) # B,40,100-->B,100-->T,100
            cur = curr_prob.transpose(0, 1).unsqueeze(0) # 1 100 T
            cur_pool = pool(cur) # 1 100 ceil(T/4) 
            cur_pool = cur_pool.squeeze(0) # 100, true_T

            Pre_av_ori = cur_pool.cpu().detach().numpy() # 100, true_T
            Pre_av = (Pre_av_ori >= 0.5).astype(np.int_) # 100, true_T

            GT_av_ori = np.load(os.path.join(label_dir, mode, video_name + '.npy')) # true_T 100
            GT_av_ori = GT_av_ori[:Pre_av.shape[1],:]
            GT_av = np.transpose(GT_av_ori) # 100,true_T

            f_av = segment_level(Pre_av, GT_av)
            F_seg_av.append(f_av)
            true_length = Pre_av.shape[1]
            event_scores = event_level_all_UnAV(Pre_av, GT_av, true_length, tiou_list=np.arange(0.1, 1.0, 0.1))
            for i, score in enumerate(event_scores):
                F_event_av[i].append(score)

            Pre_av_ori = np.transpose(Pre_av_ori) # true_T 100
            seg_map = compute_perframe_map(Pre_av_ori, GT_av_ori)
...
def main():
    ...
    parser.add_argument("--audio_dir", type=str, default='../dataset/UnAV-100/data/onepeace',
                        help="audio features dir")
    parser.add_argument("--visual_dir", type=str, default='../dataset/UnAV-100/data/onepeace',
                        help="2D visual features dir")

    parser.add_argument("--label_train", type=str, default="../dataset/UnAV-100/label/label_onepeace/train")
    parser.add_argument("--label_val", type=str, default="../dataset/UnAV-100/label/label_onepeace/val")
    parser.add_argument("--label_test", type=str, default="../dataset/UnAV-100/label/label_onepeace/test")
    parser.add_argument("--f_label_train", type=str, default="../dataset/UnAV-100/label/feature_label_modified/train")
    parser.add_argument("--f_label_val", type=str, default="../dataset/UnAV-100/label/feature_label_modified/val")
    parser.add_argument("--f_label_test", type=str, default="../dataset/UnAV-100/label/feature_label_modified/test")
    parser.add_argument("--true_label_dir", type=str, default="../dataset/UnAV-100/label/label_clip_clap")
    ...
    parser.add_argument('--val_batch_size', type=int, default=240)
    ...
    parser.add_argument('--lr_min', type=float, default=5e-6, help='the minimum lr for lr decay')

    # model hyper-parameters
    parser.add_argument("--model", type=str, default='PreFM_plus_On_AVEL_Net', help="which model to use")
    parser.add_argument("--input_v_dim", type=int, default=1536)
    parser.add_argument("--input_a_dim", type=int, default=1536)
    ...
    # args.input_a_dim = 1536
    # args.input_v_dim = 1536
    print('args =', args)
    ...
    model = PreFM_plus_On_AVEL_Net(args).to(device)

    if args.mode == 'train':

        train_dataset = PreFM_plus_UnAV_dataset(mode=args.mode, pd_dir=args.pd_dir_train, audio_dir=args.audio_dir, visual_dir=args.visual_dir,
                                    label_dir=args.label_train, f_label_dir=args.f_label_train, f_lens = args.future_length)
        val_dataset   = PreFM_plus_UnAV_dataset(mode='val', pd_dir=args.pd_dir_val, audio_dir=args.audio_dir, visual_dir=args.visual_dir,
        ...
        test_dataset = PreFM_plus_UnAV_dataset(mode='test', pd_dir=args.pd_dir_test, audio_dir=args.audio_dir, visual_dir=args.visual_dir,
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch%6==0:
                F_scores, val_loss_dict = eval(args, model, val_loader, args.true_label_dir, criterion, device, mode='val')
                ...
                print('Epoch[{}/{}](Time:{:.2f} sec)(lr:{:.6f}) Train Loss: {:.3f} Val Loss: {:.3f}  Val : {:.3f}, {:.3f}, {:.3f}'.format(
        ...
        # Evaluation
        print('begin testing...')
        F_scores_test, _ = eval(args, model, test_loader, args.true_label_dir, criterion=None, device=device, mode='test')
