...
from dataloader_PreFM import PreFM_LLP_dataset
from network_PreFM import PreFM_On_AVVP_Net
from utils.eval_metrics import segment_level, event_level_all_LLP
import datetime
...
def train(args, model, train_loader, optimizer, criterion, epoch, device):
    model.train()
    train_loss = {'total': 0, 'loss_cls': 0, 'loss_kd_curr': 0,'loss_kd_futu': 0, 'loss_all': 0}
...
def eval(args, model, data_loader, label_dir, criterion, device, mode):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            event_scores_a = event_level_all_LLP(Pre_av_t[:,0,:], GT_a_t, true_length, tiou_list=np.arange(0.1, 1.0, 0.1))
            for i, score in enumerate(event_scores_a):
                ef_a[i].append(score)
            
            event_scores_v = event_level_all_LLP(Pre_av_t[:,1,:], GT_v_t, true_length, tiou_list=np.arange(0.1, 1.0, 0.1))
            for i, score in enumerate(event_scores_v):
                ef_v[i].append(score)

            event_scores_av = event_level_all_LLP(Pre_av_t[:,0,:] * Pre_av_t[:,1,:], GT_a_t * GT_v_t, true_length, tiou_list=np.arange(0.1, 1.0, 0.1))
...
def main():
    ...
    parser.add_argument("--pd_dir_train", type=str, default='./data_info/LLP/train_pd.csv',
                        help="pd_dir_train")
    parser.add_argument("--pd_dir_val", type=str, default='./data_info/LLP/val_pd.csv',
                        help="pd_dir_val")
    parser.add_argument("--pd_dir_test", type=str, default='./data_info/LLP/test_pd.csv',
                        help="pd_dir_test")
    
    parser.add_argument("--audio_dir", type=str, default='../dataset/LLP/data/CLAP',
                        help="audio features dir")
    parser.add_argument("--visual_dir", type=str, default='../dataset/LLP/data/CLIP',
                        help="2D visual features dir")
    parser.add_argument("--st_dir", type=str, default='../dataset/LLP/data/st',
                        help="3D visual features dir")
    parser.add_argument("--label_dir", type=str, default='../dataset/LLP/label/label')
    parser.add_argument("--f_label_dir", type=str, default='../dataset/LLP/label/feature_label')
    ...
    parser.add_argument("--model", type=str, default='PreFM_On_AVVP_Net', help="which model to use")
    ...
    model = PreFM_On_AVVP_Net(args).to(device)


    if args.mode == 'train':
        train_dataset = PreFM_LLP_dataset(mode=args.mode, pd_dir=args.pd_dir_train, audio_dir=args.audio_dir, visual_dir=args.visual_dir, st_dir=args.st_dir,
                                    label_dir=args.label_dir, f_label_dir=args.f_label_dir, f_lens = args.future_length)
        val_dataset   = PreFM_LLP_dataset(mode='val', pd_dir=args.pd_dir_val, audio_dir=args.audio_dir, visual_dir=args.visual_dir, st_dir=args.st_dir,
                                    label_dir=args.label_dir, f_label_dir=args.f_label_dir, f_lens = args.future_length)
        ...
        test_dataset = PreFM_LLP_dataset(mode='test', pd_dir=args.pd_dir_test, audio_dir=args.audio_dir, visual_dir=args.visual_dir, st_dir=args.st_dir,
                                    label_dir=args.label_dir, f_label_dir=args.f_label_dir, f_lens = args.future_length)
        ...
        test_dataset = PreFM_LLP_dataset(mode='test', pd_dir=args.pd_dir_test, audio_dir=args.audio_dir, visual_dir=args.visual_dir, st_dir=args.st_dir,
                                    label_dir=args.label_dir, f_label_dir=args.f_label_dir, f_lens = args.future_length)
        test_loader = DataLoader(test_dataset, args.val_batch_size, shuffle=False, num_workers=16, pin_memory=True)
        model.load_state_dict(torch.load(os.path.join(args.model_save_dir, args.model_name, "checkpoint_best.pt")))
